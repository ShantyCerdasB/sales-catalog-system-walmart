...
import type { SignInValues } from './SignInForm'
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="w-full max-w-md p-6 bg-white rounded-lg shadow">
        {mode === 'signIn' ? (
          <SignInForm
            onSubmit={(v) => showConfirm('Proceed with sign-in?', v)}
            onSwitchToSignUp={toggleMode}
          />
        ) : (
          <SignUpForm
            onSubmit={(v) => showConfirm('Proceed with sign-up?', v)}
            onSwitchToSignIn={toggleMode}
          />
        )}
      </div>
